...
class Sim:
	...
	def countAvailableCrew(self):
		'''
		Add description of countAvailableCrew function
		'''
		return True

	def runSim(self):
		'''
		Add description of runSim function
		'''
		return True

	def runRank(self, element):
		'''
		Add description of runRank function
		'''
		return True

	# def create_outages_schedule_backup(self, outageCandidates):
	# 	'''
	...
	# 	failureProbability = np.array([x.failureProb for x in outageCandidates])
	# 	randomNumber = 1
	# 	while  (randomNumber > failureProbability).all():
	# 		randomNumber = np.random.rand()

	# 	# outages = pd.DataFrames({'failureProb':[x.failureProb for x in outage_candidates]}, index  = pd.index}
	# 	failure = np.where((randomNumber <= failureProbability), np.random.randint(0,[eventTimeSteps]*len(outageCandidates)), timeSteps)
	# 	reparationDelay = np.array([1]*len(failure))
	# 	reparationStart = failure + reparationDelay
	# 	timeToRepair = np.array([x.normalTTR for x in outageCandidates])
	# 	reparationStop = reparationStart+timeToRepair

	# 	# outageAtTimeStep = (randomNumber <= failureProbability)*np.random.randint(0,[timeSteps]*len(outageCandidates))
	# 	outagesSchedule = pd.DataFrame([[STATUS['on']]*len(outageCandidates)]*timeSteps, columns = [x.ID for x in outageCandidates], index = range(0, timeSteps))
	# 	print('failure: {}'.format(failure))
	# 	print('reparationDelay: {}'.format(reparationDelay))
	# 	print('reparationStart: {}'.format(reparationStart))
	# 	print('timeToRepair: {}'.format(timeToRepair))
	# 	print('reparationStop: {}'.format(reparationStop))
	# 	for i, column in enumerate(outagesSchedule):
	# 		outagesSchedule[column].loc[(outagesSchedule.index >= failure[i]) & (outagesSchedule.index < reparationStart[i])] = STATUS['off']
	# 		outagesSchedule[column].loc[(outagesSchedule.index >= reparationStart[i]) & (outagesSchedule.index < reparationStop[i])] =STATUS['reparing']
	# 		outagesSchedule[column].loc[(outagesSchedule.index >= reparationStop[i])] =STATUS['on']

	# 	print(outagesSchedule)
	# 	# self.add_reparing_schedule(outagesSchedule, outageCandidates)
	# 	breakpoint()
	...
	def create_outages_schedule(self, network):
		...
		for index, row in outagesSchedule.iterrows():
			...
			elementsToRepair, repairingCrews = network.get_closest_available_crews(availableCrews, failureElements)
			crewsTravelingTime = network.get_crews_traveling_time(repairingCrews, elementsToRepair)
			repairingTime = network.get_reparing_time(elementsToRepair, failureCandidates)
			...
			for t_0, t_1, e, c in zip(crewsTravelingTime, repairingTime, elementsToRepair, repairingCrews):
				...
				outagesSchedule.loc[index+t_0+t_1+1:, e] = STATUS['on'] #TODO: this line can be removed if outagesSchedule is set to 1 on at failure time

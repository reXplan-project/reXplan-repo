...
class Network:
    ...
    def build_pp_network(self, filename):
        ...
        df_bus = pd.read_excel(f'{filename}.xlsx', sheet_name=SHEET_NAME_NODES)
        df_bus = df_bus.where(pd.notnull(df_bus), None)
        bus_ids = {}
        for index, row in df_bus.iterrows():
            ...
            self.nodes.append(Bus(ID=row[COL_NAME_INDEX],
                                    lon=row[COL_NAME_LONGITUDE],
                                    lat=row[COL_NAME_LATITUDE],
...
class Bus(PowerElement):
    ...
    def __init__(self,
                 ...
                 inService=True,
                 kw=4):
        super().__init__(ID, lon, lat, fragilityCurve, kf, resilienceFull, failureProb, inReparation, normalTTR, distTTR, inService, kw)
...
class Generator(PowerElement):
    ...
    def __init__(self,
                 ...
                 inService=True,
                 kw=4):
        super().__init__(ID, geoPosition, fragilityCurve, kf, resilienceFull, failureProb, inReparation, normalTTR, distTTR, inService, kw)
        self.weatherTTR = weatherTTR

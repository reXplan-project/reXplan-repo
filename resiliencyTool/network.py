...
class Network:
    ...
    def update_montecarlo_variables(self, standard_dict):
        # TODO: updated accordingly with new definiton of montecarlo variable
    ...
    def updateGrid(self, montecarlo_database):
        for type, id, field in montecarlo_database:
    ...
    def build_timeseries_database(self, time):
        return build_database(get_datatype_elements(self, TIMESERIES_CLASS)).loc[time.start: time.stop-1]
    ...
    def run(self, time):
        ...
        return calculationEngine.run(self.build_timeseries_database(time))

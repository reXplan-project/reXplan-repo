...
def get_features():
    ...
    X_train, _ = train_test_split(X, train_size = 0.50, test_size = 0.10, stratify = pd.DataFrame(l)[0], shuffle = True, random_state = 42)
    return X_train, X_df_montecarlo, timestep_mapping_df

def get_opf_list():
    X_train, X_df_montecarlo, timestep_mapping_df = get_features()

    #idx_for_opf = pd.concat([pd.Series(X_train[:, 0]) , pd.Series(X_val[:, 0])]) UNCOMMENTED FOR THESIS
    idx_for_opf = pd.Series(X_train[:, 0]) # THESIS
...
####---- Uncomment everything below here to restore
# def get_features():
#     global X_train
#     global X_val
#     global X_df_montecarlo
...
#     # TODO make train size a variable available as input
#     X_train, X_val = train_test_split(X, train_size = 0.15, test_size = 0.15, stratify = pd.DataFrame(l)[0], shuffle = True, random_state = 42)
